...
function Word({ children, ...props }) {
  ...
  useFrame(({ camera }) => {
    ...
    // Animate font color
    ref.current.material.color.lerp(color.set(hovered ? Colors : 'white'), 0.1)
  })
...
function Cloud({ count = 4, radius = 20 }) {
  ...
  const words = useMemo(() => {
    ...
    for (let i = 1; i < count + 1; i++)
      ...
      for (let j = 0; j < count; j++) {temp.push([new THREE.Vector3().setFromSpherical(spherical.set(radius, phiSpan * i, thetaSpan * j)), rand()]); console.log(j)}
    return temp
  }, [count, radius])
